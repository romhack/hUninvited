...
module RomhackLib ( showListHex
                  ...
                  , decodeByTable, addComments, encodeByTable
...
encodeByTable encodeTbl script = concat encoded
  where
    encoded = case parse (scriptParser encodeTbl)
                    "Script parse failed" script of
      Left err -> error $ show err
      Right codes -> codes

scriptParser :: EncodeTable -> Parser [[Word8]]
scriptParser encodeTbl = many (newLines <|> comments) >> --first occurence of skipped data
  ...
  where
    ...
    comments = void $ string "//" >> anyChar `manyTill` newline
